...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        String stime = Integer.toString(hour) + ":" + Integer.toString(minute) + ":" + Integer.toString(second);
        ...
        LinearGradient linearGradient = new LinearGradient(
                ...
                Color.GRAY,Color.YELLOW, Shader.TileMode.CLAMP
        );
        needlePaint.setShader(linearGradient);
        ...
        canvas.drawText(
                stime,
                (float)(centerX - (radius+180)*Math.cos(Math.toRadians(43f))),
                (float)(centerY - (radius+100)*Math.cos(Math.toRadians(43f))),
                timePaint
        );
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            canvas.drawText(
                    number,
                    (float) (centerX + (radius * 0.8f + textWidth / 2f) * Math.cos(Math.toRadians(hourDegree))-20),
                    (float) (centerY + (radius * 0.8 + textHeight / 2f) * Math.sin(Math.toRadians(hourDegree))+15),
                    numberPaint
            );
        }
        numberPaint.setColor(Color.GRAY);
        canvas.drawCircle(centerX,centerY,10,numberPaint);
    }
